...
- do make sure to use either/or monitor setup at the top of `hyprland.conf`, comment `nvidia.conf`
...
- Add the following to your `~/.bashrc` so you can safely load `~/.bashrc.d/*.sh` dotfiles

```sh
if [ -t 0 ]; then for rc in $HOME/.bashrc.d/*.sh; do source $rc; done; fi
```
